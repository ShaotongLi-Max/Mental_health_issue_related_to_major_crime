...
By comparing the two line plots,a positive relationship between the number of MHA reports and the number of Major Crime Indicator, the number of MHA reports seems to influence the number of crime incidents accross different police divisions. In order to confirm there are linear relationship between two factors, a linear model is made to fit in this situation based on two data:
...
The p-values for both intercept and coefficient (MHA Reports) are below the significant level of 0.05, indicating a strong evidence that the relationship between MHA Reports and major crime indicator follows this linear model.
...
The residuals in this model are relatively large, leading to a complex residual pattern. This complexity may indicate potential issues with model fit or the presence of unaccounted variability in the data.
